    				...
    				<div class="col-sm-12 table-bordered">
    					<div class="main1" style="margin:1em;">
    						<div class="main1_title">
    							<div class="row">
    								<div class="col-sm-6">
            							...
            							?>
            							<label class="main_lable1"><?php echo $category_name;?></label>
    								</div>
    								<div class="col-sm-6" style="display:flex; justify-content:flex-end; align-items:center; padding-right:2vw;">
    									<select class="sort" id="" name="sort" onChange="location.href=value;" style="height:2vw; font-size:1vw;">
    										...
    										<option value="G3-1-2.php?category_id=<?php echo $_GET['category_id'];?>&page=1&select_sort=1" <?php if($select_sort==1){echo 'selected';}?>>新着順</option>
    										<option value="G3-1-2.php?category_id=<?php echo $_GET['category_id'];?>&page=1&select_sort=2" <?php if($select_sort==2){echo 'selected';}?>>価格の高い順</option>
    										<option value="G3-1-2.php?category_id=<?php echo $_GET['category_id'];?>&page=1&select_sort=3" <?php if($select_sort==3){echo 'selected';}?>>価格の低い順</option>
    									</select>
    								</div>
    							</div>
    						</div>
    						...
    						<div class="row">
    							...
    							$sql = "select * from product inner join series on product.series_id = series.series_id where category_id = ? ";
                                ...
                                $check = $sql->execute([$_REQUEST['category_id']]);
                                $sql2 = $pdo -> prepare("select *,count(*) as count from product inner join series on product.series_id = series.series_id where category_id = ?");
                                $sql2 -> execute([$_GET['category_id']]);
    						...
    						<div class="" style="text-align:center;">
    							<ul class="pagination">
                                  <li><a href="G3-1-2.php?category_id=<?php echo $_GET['category_id'];?>&page=<?php if($_REQUEST['page']!=1){echo $_REQUEST['page']-1;}else{echo '1';}?><?php if(isset($select_sort)){echo '&select_sort='.$select_sort;}?>">«</a></li>
                                  <li><a class="<?php if($_REQUEST['page']==1){echo 'active';}?>" href="G3-1-2.php?category_id=<?php echo $_GET['category_id']?>&page=1<?php if(isset($select_sort)){echo '&select_sort='.$select_sort;}?>">1</a></li>
                                  <?php
                                  $sql = $pdo -> prepare("select *,count(*) as count from product inner join series on product.series_id = series.series_id where category_id = ?");
                                  $sql -> execute([$_GET['category_id']]);
                                  ...
                                  foreach($sql as $row){
                                    ...
                                    if($pages > 1){
                                        for($i = 2; $i<=$pages; $i++){
                                            echo '<li><a class="'?><?php if($_REQUEST['page']==$i){echo 'active';}?><?php echo'"href="G3-1-2.php?category_id='?><?php echo $_REQUEST['category_id'];?><?php echo '&page='?><?php echo $i;?><?php if(isset($select_sort)){echo '&select_sort=',$select_sort;};?><?php echo '">';
                                            echo $i;
                                            echo '</a></li>';
                                        }
                                    }
                                  ...
                                  ?>
                                  <li><a href="G3-1-2.php?category_id=<?php echo $_GET['category_id'];?>&page=<?php if($_REQUEST['page']<$pages){echo $_REQUEST['page']+1;}else{echo $_GET['page'];}?><?php if(isset($select_sort)){echo '&select_sort=',$select_sort;};?>">»</a></li>
                                </ul>
    						</div>
    						<!-- ページ変更画面(ページネーション) 終了 -->
    					</div>
