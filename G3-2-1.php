...
<?php //閲覧履歴DB(History)に追加する
    if(isset($_SESSION['customer'])){
        ...
        $sql = $pdo->prepare('select count(*) as current_count from history where user_id = ? and product_id = ?');
        ...
        if($row['current_count']==1){//カート内に同一商品がある場合(current_countの出力が1以上)
            $sql = $pdo -> prepare('update history set history_date = ? where user_id = ? and product_id = ?');//更新SQL
            $sql ->execute([$today,$_SESSION['customer']['user_id'],$_GET['product_id']]);
				...
				<div class="container">
					<div class="row" style="min-height:100%;">
						<div class="col-sm-7" style="min-height:100%;">
							...
							<div class="row mt-0">
								<?php
									$pdo1 = new PDO('mysql:host=localhost; dbname=we will shopping; charset=utf8','root');
						...
						<div class="col-sm-5" style="min-height:100%;">
							...
							<div class="row" style="height:30%;">
								<div class="star_image_container col-sm-6 form_container">
      								...
      								<a href="#motal1" class="modal btn btn-tag btn-tag--favorite">
      									<i class="fas fa-star" style="margin-right:15%;"></i>
      									<label style="margin:0; font-size:1vw;">お気に入りに追加</label>
      								</a>
								...
								<div class="col-sm-6 form_container h-100">
									<select class="count_selectbox" id="quantity" name="quantity" style="height:100%; font-size:1vw;">
                        							<?php
...
<!--        カスタマーレビュー開始 -->
		<div id="review" class="review_container" style="width:95%; margin:0 auto;">
			...
			<div class="row" style="width:90%; height:3vw; margin:0 auto;">
				...
				<div class="col-sm-6" style="display:flex; justify-content:flex-end; align-items:center; margin:0 0 0 auto;">
					<select class="sort" name="sort" style="width:30%; height:2vw; font-size:1vw;">
						<option value="" selected>トップレビュー</option>
					</select>
				</div>
			</div>
			<div class="row" style="width:90%; margin:0 auto;">
