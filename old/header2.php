  ...
  <title></title>
    ...
    function show_cards(){//支払方法のradioボタン判定(カード支払い、を押した時)
    	if(cards.style.display=="none"){
    		...
    		card.setAttribute("required", "");		//required属性を追加する
    		card.required = true;
    		// noneで非表示
    ...
    function close_cards(){//支払方法のradioボタン判定(現金支払い、を押した時)
    	if(cards.style.display=="block"){
    		var card = document.getElementById("card");
			card.removeAttribute("required");		//required属性を取り除く
    		card.required = false;
    		// noneで非表示
...
<body>
	...
	<nav class="navbar navbar-expand-md navbar-dark bg-dark sticky-top" style="height:4vw;">
      ...
      <ul class="navbar-nav" style="display: flex; justify-content: center; align-items: center;">
		<li class="nav-item ">
            <a href="Mainpage.php">
            	<i class="logo-right fas fa-home"></i>
            </a>
        </li>
        <li class="nav-item ">
            <a href="G5-1-1.php">
